...
func run_checks(_tree: SceneTree) -> int:
	print("=== WM_003 THE LAST PERFORMANCE CONTENT & LOOP VALIDATION ===")
	...
	var incident := registry.get_incident("INC_LAST_PERFORMANCE")
	_check("I1 INC_LAST_PERFORMANCE loaded from registry", incident != null)
	if incident == null:
		return _finish()

	_check("I2 incident title correct", incident.title == "The Last Performance")
	_check("I3 incident primary memory case correct", incident.primary_memory_case_id == "MC_LAST_PERFORMANCE")

	var director := WitnessExperienceDirector.new()
	get_tree().root.add_child(director)
	var moment := director.select_moment("WM_003")
	_check("M1 WM_003 loaded successfully", moment != null)
	if moment == null:
		director.queue_free()
		return _finish()

	_check("M2 moment title correct", moment.title == "The Last Performance")
	...
	var selection := director.get_next_incident({"mode": "story", "incident_id": "INC_LAST_PERFORMANCE"})
	orchestrator.start_incident(selection)
	await get_tree().process_frame
	await get_tree().process_frame

	_check("S1 orchestrator started INC_LAST_PERFORMANCE successfully", orchestrator.is_active())
...
func _finish() -> int:
	print("=== WM_003 VALIDATION RESULT: %d passed, %d failed ===" % [_passed, _failed])
	return _failed
