...
func _draw_fibers(center: Vector2, radius: float, pupil_ratio: float, motion: float, density: int, presence: float, glow: float, focus: float, drift: float, asymmetry: float) -> void:
	...
	for index in range(density):
		...
		var seed := float(index) * 1.618 + asymmetry
		var angle := ratio * TAU + sin(elapsed * 0.29 + seed) * 0.050 * motion
		var direction := Vector2(cos(angle), sin(angle))
		var tangent := Vector2(-direction.y, direction.x)
		var organic := sin(seed * 2.13 + elapsed * (0.51 + motion * 1.37))
		organic += sin(seed * 0.71 - elapsed * 0.17) * 0.34
		var inner := inner_base * (0.91 + 0.16 * sin(seed * 1.63))
		var outer := radius * (0.62 + 0.27 * (sin(seed * 0.79 + drift) * 0.5 + 0.5))
		var first_bend := radius * (0.012 + motion * 0.044) * organic
		var second_bend := radius * (0.008 + motion * 0.030) * sin(seed * 3.73 - elapsed * 0.41)
		...
		var bright := 0.32 + 0.68 * (sin(seed * 4.17 + elapsed * 1.09 + drift) * 0.5 + 0.5)
